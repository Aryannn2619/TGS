...
export default function JoinButton({ challengeId }: { challengeId: string }) {
  ...
  async function start() {
    ...
    const orderRes = await fetch('/api/payments/create-order', {
      ...
      body: JSON.stringify({ paymentType: 'challenge', challengeId }),
    });
    const order = await orderRes.json();
    if (!orderRes.ok) { setErr(order.error || 'Could not create order'); setLoading(false); return; }

    if (order.mockMode) {
      // Mock: skip Razorpay UI, post a fake verify directly.
    ...
    if (!order.razorpayKeyId) { setErr('Razorpay key not configured'); setLoading(false); return; }
  ...
  return (
    <div className="flex flex-col items-end gap-2">
      <button onClick={start} disabled={loading} className="btn-primary">
        {loading ? 'Working…' : 'Unlock Challenge'}
      </button>
